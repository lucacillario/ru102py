...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(self, meter_reading: MeterReading,
        ...
        # START Challenge #6
        reading = MeterReadingSchema().dump(meter_reading)
        pipeline.xadd(self.key_schema.global_feed_key(), reading, maxlen=self.GLOBAL_MAX_FEED_LENGTH)
        pipeline.xadd(self.key_schema.feed_key(meter_reading.site_id), reading, maxlen=self.SITE_MAX_FEED_LENGTH)
        # END Challenge #6
